...
def main():
    ...
    parser.add_argument("--no-jitter", action="store_true", help="Skip the anti-detection random timing jitter (for immediate testing)")
    ...
    if hasattr(args, "post_now") and args.post_now:
        ...
        run_slot_pipeline(slot, config, logger, apply_jitter=apply_jitter)
        return
    ...
    if args.test_facebook:
        ...
        res = publisher.publish_full_campaign(
            caption_text="Test 4-channel post from Maira Dash automated agent with safety guardrails! 💖✨\n\n#Testing #Automation",
            image_path=test_img,
    ...
    if args.run_scheduler:
        def on_trigger(slot_key):
            return run_slot_pipeline(slot_key, config, logger, apply_jitter=True)

        scheduler = PostScheduler(config, on_trigger)
        scheduler.start()
        return

    # Interactive menu if no arguments passed (manual runs run without jitter for immediate feedback)
    print("\n" + "=" * 68)
    ...
    print(" 1. Start 24/7 Python Scheduler (In this terminal - with jitter)")
    print(" 2. Post 'Good Morning' now (Immediate - All 4 channels)")
    print(" 3. Post 'Good Noon' now (Immediate - All 4 channels)")
    print(" 4. Post 'Good Afternoon' now (Immediate - All 4 channels)")
    print(" 5. Post 'Good Evening' now (Immediate - All 4 channels)")
    print(" 6. Post 'Good Night' now (Immediate - All 4 channels)")
    ...
    if choice == "1":
        ...
        run_slot_pipeline(slot_map[choice], config, logger, apply_jitter=False)
    elif choice == "7":
        slot = "morning"
        slot_info = config["slots"][slot]
        chatgpt = ChatGPTAgent(config)
