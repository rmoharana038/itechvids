...
def human_delay(min_sec=1.5, max_sec=3.5):
    """Sleeps for a randomized duration to simulate human reaction and thinking time."""
...
class FacebookPublisher:
    def __init__(self, config):
        ...
        self.enable_human_delays = self.safety_cfg.get("enable_human_delays", True)
        self.enable_checkpoint_detection = self.safety_cfg.get("enable_checkpoint_detection", True)
    ...
    def check_security_checkpoint(self, page, context_name=""):
        """
        Scans for security checkpoints, identity verification prompts, or temporary action blocks.
        If detected, takes a screenshot and returns True so the agent can safely abort.
        """
        if not self.enable_checkpoint_detection:
            return False

        try:
            current_url = page.url.lower()
            if any(chk in current_url for chk in ["/checkpoint/", "/recover/", "/login/device-based/"]):
                logger.critical(f"[SECURITY ALERT] Facebook checkpoint URL detected during {context_name}: {page.url}")
                page.screenshot(path="logs/security_checkpoint.png")
                return True
            ...
            for phrase in checkpoint_phrases:
                if phrase in body_text:
                    logger.critical(f"[SECURITY ALERT] Facebook checkpoint phrase detected ('{phrase}') during {context_name}!")
                    page.screenshot(path="logs/security_checkpoint.png")
                    return True
        except Exception:
            pass
    ...
    def human_click(self, locator, timeout=5000):
        """Simulates natural human hover before clicking."""
        try:
    ...
    def ensure_context(self, page, target_type="page"):
        ...
        Strictly checks and verifies via https://www.facebook.com/me with anti-detection pacing.
        """
        ...
        if self.check_security_checkpoint(page, f"navigating to /me for {target_type}"):
            return False
        ...
        if self.check_security_checkpoint(page, f"re-verifying {target_type}"):
            return False
    ...
    def publish_full_campaign(self, caption_text, image_path, slot_info, location=None, ai_label=None):
        ...
        logger.info(f"Anti-Detection: Human delays={self.enable_human_delays}, Checkpoints={self.enable_checkpoint_detection}")
        logger.info("=================================================================")
        ...
        with sync_playwright() as p:
            ...
            try:
                ...
                if not page_switched:
                    logger.error("Could not switch to Page profile or security checkpoint triggered. Aborting campaign.")
                    context.close()
                    return {"success": False, "error": "Could not switch to Page profile or checkpoint detected", "details": results}
                ...
                # -------------------------------------------------------------
                if self.default_share_to_story and results["page_feed"]:
                    ...
                    logger.info("Stage 2 skipped: share_to_story disabled in config or feed post failed.")

                # Natural human breathing pause between Page and Profile operations
                human_delay(4.0, 7.0)
                ...
                if self.post_to_profile:
                    ...
                    if not profile_switched:
                        logger.error("Could not switch/verify Personal Profile persona or checkpoint detected! Skipping profile feed post.")
                    else:
                ...
                # -------------------------------------------------------------
                if self.share_to_profile_story and results["profile_feed"]:
                    ...
                    logger.info("Stage 4 skipped: share_to_profile_story disabled or profile feed failed.")
    ...
    def _publish_feed_post(self, page, target_name, caption_text, image_abs_path, feeling, location, ai_label):
        ...
        Uses anti-detection humanized delays and checkpoint checks.
        """
        ...
        if self.check_security_checkpoint(page, f"{target_name} home feed"):
            return {"success": False, "error": "Security checkpoint detected"}
        ...
        if composer_span.is_visible():
            self.human_click(composer_span)
        else:
        ...
        logger.info("Waiting for photo upload preview and processing...")
        human_delay(6.0, 8.5)
    ...
    def publish_story(self, image_path, ai_label=True, page=None, context_name="Story"):
        ...
        def _do_upload(p_page):
            ...
            if self.check_security_checkpoint(p_page, f"{context_name} story create"):
                return False

            file_inp = p_page.locator('input[type="file"]').first
            if file_inp.count() > 0:
                file_inp.set_input_files(image_abs_path)
                logger.info(f"Photo attached to {context_name} story editor. Waiting for preview...")
                human_delay(5.0, 7.0)
                ...
                if share_btn.is_visible():
                    ...
                    logger.info(f"Clicked 'Share to Story' for {context_name}. Waiting for upload...")
                    human_delay(9.0, 12.0)
    ...
    def publish_post(self, caption_text, image_path, slot_info=None, location=None, ai_label=None):
        """Helper alias to publish_full_campaign."""
        if slot_info is None:
            slot_info = {"feeling": "blessed", "label": "Post"}
        return self.publish_full_campaign(caption_text, image_path, slot_info, location, ai_label)
